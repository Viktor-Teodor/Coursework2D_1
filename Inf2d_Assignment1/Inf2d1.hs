...
depthFirstSearch destination next branches exploredList
  |checkArrival destination (head (last branches)) = if not (null (head branches)) then Just (head branches) else Nothing
  |let expansion = next (last branches) in null expansion = Nothing
  |otherwise = depthFirstSearch destination next (init (branches ++ filter(\aux -> notElem (head aux) exploredList) (next (last branches)))) (head (last branches) :exploredList)
...
depthLimitedSearch destination next branches d
  |checkArrival destination (head (last branches)) = if not (null (head branches)) then Just (head branches) else Nothing
  |let expansion = next (last branches) in null expansion = Nothing
  |otherwise = if length (last branches) <d then depthLimitedSearch destination next (init (branches ++ (next (last branches)))) d else depthLimitedSearch destination next (init (branches)) d
